...
def __init_logger(log_file=None, log_file_level=logging.INFO):
    global logger
    if logger is None:
        logger = init_logger(log_file, log_file_level)
...
def __train_abs_multi(args, device_ids):
    ...
    for i, device_id in enumerate(device_ids):
        procs.append(mp.Process(target=__run, args=(args,
                                                    device_id, error_queue,), daemon=True))
        procs[i].start()
        logger.info(f'Starting process pid: {procs[i].pid}, device_id: {device_id}')
        error_handler.add_child(procs[i].pid)
    for p in procs:
        p.join()


def __run(args, device_id, error_queue):
    """ run process """
    ...
    try:
        ...
        logger.info(f'gpu_rank: {gpu_rank}, args.gpu_ranks: {args.gpu_ranks}, device_id: {device_id}, world_size: {args.world_size}')
        if gpu_rank != args.gpu_ranks[device_id]:
            raise AssertionError("An error occurred in \
                  Distributed initialization")

        __train_abs_single(args, device_id)
...
def validate(args, device_id, pt, step):
    ...
    if args.tokenizer == "multi":
        ...
        tokenizer = BertKoreanMecabTokenizer(args.vocab, joining=True)
    ...
    return stats.xent(), stats.accuracy()


def test_abs_all(args, device_id, data='test'):
    cp_files = glob.glob(os.path.join(args.model_path, 'model_step_*.pt'))
    ...
    for i, cp in enumerate(cp_files):
        ...
        test_abs(args, device_id, cp, step, data=data)
...
def train_abs(args, device_ids):
    ...
    if (args.world_size > 1):
        ...
        __train_abs_single(args, device_ids[0])
...
def __train_abs_single(args, device_id):
    ...
    trainer.train(__train_iter_fct, args.train_steps)
