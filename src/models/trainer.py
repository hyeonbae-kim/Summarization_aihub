...
class Trainer(object):
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
        ...
        for batch in true_batchs:
            ...
            def_logger.debug(
                f'src.shape: {src.shape}, tgt.shape: {tgt.shape}, segs.shape: {segs.shape}, clss.shape: {clss.shape}, mask_src.shape: {mask_src.shape}, mask_tgt.shape: {mask_tgt.shape}, mask_cls.shape: {mask_cls.shape}')

            outputs, scores = self.model(src, tgt, segs, clss, mask_src, mask_tgt, mask_cls)
            def_logger.debug(f'outputs.shape: {outputs.shape}, scores.shape: {scores.shape if scores is not None else "None"}')

            batch_stats = self.loss.sharded_compute_loss(batch, outputs, self.args.generator_shard_size, normalization)
            def_logger.debug(f'batch_stats: {batch_stats}, generator_shard_size: {self.args.generator_shard_size}, normalization: {normalization}')
    ...
    def test(self, test_iter, step, cal_lead=False, cal_oracle=False):
        ...
        if (step != -1 and self.args.report_rouge):
            ...
            def_logger.info('Rouges at step %d \n%s' % (step, rouge_results_to_str(rouges)))
        self._report_step(0, step, valid_stats=stats)
