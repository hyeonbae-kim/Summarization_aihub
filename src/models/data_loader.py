...
class Batch(object):
    ...
    def __init__(self, data=None, device=None, is_test=False):
        ...
        if data is not None:
            ...
            # preprocess() 처리된 결과 순서 : src, tgt, segs, clss, src_sent_labels [, src_txt, tgt_txt]
            ...
            # cls(3) 토큰이 존재하는 위치 예) [0, 43, 88, 175]
...
def load_dataset(args, corpus_type, shuffle):
    ...
    # {dataset}_{subdataset}.{corpus_type} 형태 지원 (예. report_novel.train.19.bert.pt)
    globs = [glob.glob(path.strip() + '*.' + corpus_type + '.[0-9]*.pt') for path in args.bert_data_path.split(",")]
    pts = sorted(sum(globs, []))
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        src = ex['src']
        tgt = ex['tgt'][:self.args.max_tgt_len][:-1] + [ex['tgt'][-1]]
    ...
    def batch(self, data, batch_size):
        ...
        for ex in data:
            minibatch.append(ex)
            # 가급적 batch_size 만큼 처리, GPU 메모리 문제가 발생할 경우 batch_size, accum_count 등 조정
    ...
    def create_batches(self):
        """ Create batches """
        data = self.data()
        for buffer in self.batch_buffer(data, self.batch_size * 1000):
            if (self.args.task == 'abs'):
            ...
            minibatch = self.batch(p_batch, self.batch_size)
