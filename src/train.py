...
if __name__ == '__main__':
    ...
    if (args.task == 'abs'):
        if (args.mode == 'train'):
            ...
            if args.test_all:
                test_abs_all(args, device_ids[0], data='valid')
            else:
                cp = args.test_from
                try:
                    step = int(cp.split('.')[-2].split('_')[-1])
                    ...
                    step = 0
                test_abs(args, device_ids[0], cp, step, data='valid')
            ...
            try:
                ...
                step = 0
                test_text_abs(args, device_ids[0], cp, step)

    elif (args.task == 'ext'):
        if (args.mode == 'train'):
            train_ext(args, device_ids)
        elif (args.mode == 'validate'):
            validate_ext(args, device_ids)
        if (args.mode == 'test'):
            cp = args.test_from
            try:
                step = int(cp.split('.')[-2].split('_')[-1])
                ...
                step = 0
            test_ext(args, device_ids, cp, step)
        elif (args.mode == 'test_text'):
            cp = args.test_from
            try:
                step = int(cp.split('.')[-2].split('_')[-1])
                ...
                step = 0
                test_text_abs(args, device_ids, cp, step)
